  ...
  const sortTimePassedDesc = (docu) => {
    if (docu && Object.keys(docu).length > 0) {
      const sortedEntries = Object.entries(docu).sort(([, a], [, b]) => {
        const parsedA = a.data.submittedBy ? a.data.submittedBy.split('|')[1] : null;
        const parsedB = b.data.submittedBy ? b.data.submittedBy.split('|')[1] : null;
    ...
    <div className="w-full h-[90%] rounded-lg">
      <div className='w-full h-full flex flex-col rounded-lg'>
        <div className='w-full h-auto hidden sm:flex items-center justify-center px-2 py-2 rounded-lg bg-gray-100 text-gray-400 text-sm'>
            ...
            {activeTabs !== 'Drafting' && activeTabs !== '' && !activeTabs.includes('Returned') && <h1 className='lg:text-sm 2xl:text-base w-1/6 text-center font-semibold'>Time Transferred</h1>}
        ...
        {isModalOpen && (
          <>
            ...
            <div className="fixed z-30 left-0 top-0 w-full h-full flex items-center justify-end">
              <DisbursementVoucher modal={modal} flag={false} tab='To Release'/>
            </div>
          </>
        )}
      </div>
